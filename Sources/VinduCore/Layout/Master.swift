...
/// Master layout: a primary area plus a stack, controlled by typed actions and
/// the public runtime dispatcher surface.
public final class MasterLayout {
    public private(set) var windows: [WindowID] = []
    public private(set) var masterCount = 1
    /// Runtime overrides set by layout actions; nil uses the current configuration.
    public private(set) var primaryFractionOverride: Double?
    ...
    public func insert(_ w: WindowID, configuration: MasterConfiguration) {
        guard !contains(w) else { return }
        switch configuration.newWindowPosition {
        case .primary:
    ...
    public func setPrimaryFraction(_ arg: SplitRatioArg, configuration: MasterConfiguration) {
        setPrimaryFraction(arg, fallback: configuration.primaryFraction)
    }

    private func setPrimaryFraction(_ arg: SplitRatioArg, fallback: Double) {
        let current = primaryFractionOverride ?? fallback
        switch arg {
        case .delta(let d): primaryFractionOverride = clamp(current + d)
        case .exact(let v): primaryFractionOverride = clamp(v)
        }
    }
    ...
    public func cycleOrientation(prev: Bool, configuration: MasterConfiguration) {
        cycleOrientation(prev: prev, fallback: configuration.primaryPosition)
    }

    private func cycleOrientation(prev: Bool, fallback: MasterOrientation) {
        let ring: [MasterOrientation] = [.left, .top, .right, .bottom, .center]
        let current = orientationOverride ?? fallback
        let idx = ring.firstIndex(of: current) ?? 0
        let n = ring.count
        orientationOverride = ring[((idx + (prev ? -1 : 1)) % n + n) % n]
    }

    public func frames(in rect: CGRect, configuration: MasterConfiguration) -> [WindowID: CGRect] {
        frames(in: rect, primaryFraction: configuration.primaryFraction,
               primaryPosition: configuration.primaryPosition)
    }

    private func frames(in rect: CGRect, primaryFraction: Double,
                        primaryPosition: MasterOrientation) -> [WindowID: CGRect] {
        guard !windows.isEmpty else { return [:] }
        let m = min(max(masterCount, 1), windows.count)
        let fraction = clamp(primaryFractionOverride ?? primaryFraction)
        let orient = orientationOverride ?? primaryPosition
