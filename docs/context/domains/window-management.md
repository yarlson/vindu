...
Every private symbol is resolved at runtime from the fixed SkyLight framework path. Notification registration and removal keep the engine context alive for the full callback lifetime. Invalid geometry hides the border without reaching WindowServer; a reported setup, transaction, drawing, or runtime failure disables only the border and emits one diagnostic. A private ABI change that preserves the symbol names can still affect the in-process daemon, so each supported macOS version needs live validation. There is no AppKit border fallback, polling loop, external border process, or direct SkyLight link.
